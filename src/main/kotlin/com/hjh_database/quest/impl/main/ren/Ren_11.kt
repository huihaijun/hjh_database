...
class Ren_11 : QuestBase("main_ren_11", "[人族主线]圣山之托", QuestType.MAIN, 25) {

    override val raceLimit = 2
    override val requiredCompletedQuestIds = setOf("main_ren_10")
    ...
    private val kaigeScript = listOf(
        "§e[${StoryNpcs.KAIGE.displayName}§e] §f你好，小友。找我何事？",
        "§e[${StoryNpcs.KAIGE.displayName}§e] §f……啊，是§e法海§f让你来的。原来如此。",
        "§e[${StoryNpcs.KAIGE.displayName}§e] §f不过他也太过多虑了。你既是§e圣兽所认定§f之人，我们又怎会为难于你。",
        ...
        "§e[${StoryNpcs.KAIGE.displayName}§e] §f如今你既持祝福而来，这§e重责大任§f，便托付与你了。前往§e圣山§f吧，想办法让§4§n四圣兽§f重归本位，令天地秩序回到正轨。",
        "§e[${StoryNpcs.KAIGE.displayName}§e] §f待你调查完圣山，便去皇宫找§e李公公§f复命。想来他也一直在等你的消息。",
    ...
    private fun giveHighTierTwinPills(player: Player) {
        val pills = plugin.resourceManager.getItem(TWIN_PILL_ID)
        if (pills == null) {
            plugin.logger.warning("人族第十一章缺少奖励物品配置: $TWIN_PILL_ID")
